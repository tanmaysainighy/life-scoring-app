...
export const GET = route(async ({ user, request, params }) => {
  if (!(await isMember(params.id, user.id))) return fail("You're not a member of that group.", 403, "forbidden");
  ...
  return ok({
    period,
    leaderboard: await getGroupLeaderboard(params.id, period, localDay(new Date(), user.timezone)),
  });
});
